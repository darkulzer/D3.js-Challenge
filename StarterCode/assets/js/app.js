...
let [svgWidth, svgHeight] = [3*window.innerWidth/4, 3*window.innerHeight/4];
...
// Import data from the data.csv file, call the function healthData
  d3.csv("./assets/data/data.csv").then((healthData, error)=>{
...
// Defining the circles on the chart
	chart.selectAll("circle")
		...
		.enter().append("circle")
			...
			.on("mouseout", (data) => {toolTip.hide();})
			//toolTip.on("mouseout", (data)=>{toolTip.hide(data);})
